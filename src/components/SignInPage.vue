  ...
  <div class="login-page">
    <div class="form-container">
      <div v-if="signinForm">
        ...
        <p>Back to  <button @click="signIn">Login</button>  </p>
      </div>
      <div v-if="loginForm" >
        <h2>Login</h2>
        <form >
       ...
       <p>Click to <button @click="signIn">Sign IN</button></p>
      </div>
    </div>
  </div>
  <Footer />
...
const handleSubmit = async () => {
  ...
  setTimeout(() => {
    toast.add({
      severity: "success",
      summary: `Sign in successful ${form.name}`,
      detail: `Sign in successful ${form.name}`,
      group: "br",
      life: 3000,
    });
    loading.value = false;
